...
{
    [SerializeField] private AudioClip Coin, Jump, JumpClip;
    ...
    private bool isGravitySlow = false;
    private bool isControlsInverted = false;
    private float invertedControlsTimer = 0f;
    private const float invertedControlsDuration = 30f;
    ...
    public void StartGame()
    {
        ...
        // Notificar al enemigo que el juego ha comenzado
        FindObjectOfType<EnemyFollow>().StartGame();
    }
    ...
    void Update()
    {
        // Gravedad lenta con tecla "J"
        if (Input.GetKeyDown(KeyCode.J))
        {
            isGravitySlow = !isGravitySlow;
            if (isGravitySlow)
            {
                Physics.gravity = new Vector3(0, -4.9f, 0); // Gravedad reducida
                Time.timeScale = 0.5f; // Movimiento lento
            }
            else
            {
                Physics.gravity = new Vector3(0, -9.8f, 0); // Gravedad normal
                Time.timeScale = 1.0f; // Movimiento normal
            }
        }
        ...
        {
            if (Input.GetKeyUp(KeyCode.A)) MoveRight(); // Invertido
            if (Input.GetKeyUp(KeyCode.D)) MoveLeft();  // Invertido
            if (Input.GetKeyUp(KeyCode.W)) Slide();     // Invertido
            if (Input.GetKeyUp(KeyCode.S)) PJump();      // Invertido
        }
        if (isControlsInverted)
        {
            invertedControlsTimer -= Time.deltaTime;

            // Si el tiempo se agota, restablecer los controles
            if (invertedControlsTimer <= 0)
            {
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            Debug.Log("PLAYER_BASE está atrapado en un obstáculo.");

            // Congelar la posición del jugador
            GetComponent<Rigidbody>().constraints = RigidbodyConstraints.FreezePosition;

            // Notificar al enemigo que el jugador está atrapado
            FindObjectOfType<EnemyFollow>().TrapPlayer();
        }
    }
